...
/*
void swape(char namaTugas[j], char namaTugas[j+1]) 
{ 
...
void bubble(int prioritas[],char namaTugas[],int n){
	int i = 1;
	...
	while (i < n){
		...
		while(j < n-i){
			if (prioritas[j]>prioritas[j+1] ){
				...
				swapper(namaTugas[j],namaTugas[j+1])
				/*
				string semen = namaTugas[j]; 
    			namaTugas[j] = namaTugas[j+1]; 
				namaTugas[j+1] = semen; */
